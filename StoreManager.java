...
public class StoreManager {
    ...
    /**
    *Default constructor of store.StoreManager
    */
    public StoreManager() {
    ...
    *@return store.Inventory, the StoreManagers inventory
    */
    ...
    *@param cartCheck store.ShoppingCart, the shopping cart that is being checked
    ...
    */
    public boolean verifyCheckOut(ShoppingCart cartCheck) {
        ...
        for (Product c : cartCheck.getItems()) {
            if (cartCheck.getCartAmount(c.getItemID()) > inventory.getProductQuantity(c)) {
                System.out.println("There is an insufficient amount of " + c.getItemName());
                System.out.println("Current stock: " + inventory.getProductQuantity(c) + ", Your request: " + cartCheck.getCartAmount(c.getItemID()));
                checkStatus = false;
            }
        }
        return checkStatus;
    ...
    *@param cart store.ShoppingCart, the cart to be checked
    ...
    */
        ...
        for (Product c: cart.getItems()) {
            for (Product p : inventory.getProductList()) {
                if (c.getItemID() == p.getItemID() && c.getItemName().equals(p.getItemName())){
                    inventory.removeProductQuantity(c, cart.getCartAmount(c.getItemID()));
    ...
    /**
    *Initilizes a new store.ShoppingCart and gives it a uniqueID
    *@return store.ShoppingCart, the new shopping cart
    */
